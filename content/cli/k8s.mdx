...
<C i={`kubectl run nginx --image=nginx`}>Démarre un pod en téléchargeant l'image de nginx depuis le dépôt public de Docker.</C>

<C i={`kubectl get pods`}>Affiche la liste de tous les pods en cours d'éxecution.</C>

<C i={`kubectl describe pod nginx`}>Affiche des informations détaillées sur un pod.</C>
